...
/*
static char fts_version[64]="ft6336u ver 1.0";
static char fts_vendor_name[64]="ft6336u";
DEV_ATTR_DECLARE(touch_screen)
DEV_ATTR_DEFINE("version",fts_version)
DEV_ATTR_DEFINE("vendor",fts_vendor_name)
...
*****************************************************************************/
#if 0
void ontim_update_fw_ver(struct i2c_client *client)
{
    u8 fwver = 0;
    int cnt=0;
    ...
    do {
        if (fts_i2c_read_reg(client, FTS_REG_FW_VER, &fwver) >= 0) {
            if ((fwver != 0xFF) && (fwver != 0x00)) {
                snprintf(fts_version, sizeof(fts_version),"ft6336u ver %d.0",fwver );
                break;
            }
        }
        cnt++;
