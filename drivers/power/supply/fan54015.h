...
struct fan54015_platform_data {
	...
	u32 chg_dis_gpio;//zhangbing@uniscope_drv 20160708 add for apply for disable gpio
	u32 chg_dis_gpio_flags;//zhangbing@uniscope_drv 20160708 add for apply for disable gpio
};

/*zhangbing@uniscope_drv 20160708 add begin*/
struct sprd_ext_ic_operations {
		void (*ic_init)(void);
		void (*charge_start_ext)(int cur);
		void (*charge_stop_ext)(void);
		int (*get_charging_status)(void);
		int (*get_charging_fault)(void);
		int (*get_charging_vbus_valid)(void);
		void (*timer_callback_ext)(void);
		void (*otg_charge_ext)(int enable);
		int (*get_charge_cur_ext)(void);
		void (*set_termina_cur_ext)(int cur);
		void (*set_termina_vol_ext)(int vol);
		int (*get_chgcur_step)(int cur,int increase);
...
extern BYTE fan54015_get_monitor_val(void);
